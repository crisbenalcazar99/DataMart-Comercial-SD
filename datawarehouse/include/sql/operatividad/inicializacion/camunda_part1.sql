SELECT 
	 ...
	 CASE WHEN t.id_proceso NOT IN (491, 495) THEN c7.numero_vigencia ELSE fs_rec.vigencia end AS vigencia, 
    ...
    CASE WHEN u.email IS NOT NULL THEN u.email ELSE ds.correo end AS correo, 
	 ...
	 u.apellido_materno,
	 CASE 
    WHEN t.id_proceso NOT IN (491, 495) 
        THEN NULLIF(fs.fecha_expiracion, '')::timestamp
    ELSE NULLIF(fs_rec.fecha_expiracion, '')::timestamp
...
JOIN PUBLIC.firma_subida fs_rec ON s.id_renovar = fs_rec.id_firma
